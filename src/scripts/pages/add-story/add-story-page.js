import { postGuestStory } from '../../data/api';
  ...
  async afterRender() {
    ...
    cameraBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const message = err.name === 'NotAllowedError' ? 'Anda telah memblokir akses kamera. Mohon izinkan melalui pengaturan browser.' : 'Gagal mengakses kamera. Pastikan Anda memberikan izin.';
        alert(message);
      }
    });
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        const res = await postGuestStory(formData);
        if (!res.error) {
          alert(res.message);
